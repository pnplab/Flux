...
// Remote object doesn't exist!` error inside the viewer. Had these two cases:
// - Restarting emulator / viewer wont do! Instead, you have to first restart
//   adb server cf. `$ adb kill-server; adb start-server`, and then only restart
//   `uiautomatorviewer`. @warning seems like this stops the instrumentation
//   testing.
// - Restarting uiautomatorviewer worker but only but when test were not
//   running. It was failing again as soon as put inside a ongoing test context.
//   This could have been because of android time setting change during the
//   test, but tested restart for this as well! Upper option (adb kill) works
//   in all scenario!
public class TaskReminderTestHelper {
    ...
    public void notifyMustServeJs() {
        ...
        Log.w("UiTest", "Instrumentation testing.");
        Log.w("UiTest", "@warning make sure you're serving the project to see the development javascript version.");
        Log.w("UiTest", "         `yarn backend & ; yarn backend:ngrok & ; yarn androidstudio & ; yarn serve`");
        Log.w("UiTest", "         if you don't, you wont be warned but will be served the lastly built js version!");
    }

    public void notifyFirstTestRunFailsBecauseJsServeIsSlow() {
        // Warn developer to launch test a second time after
        // `npx react-native serve` had the time to load.
        Log.w("UiTest", "@warning First served JS build is always very slow. First test launch is thus likely to fail.");
    }
    ...
    public void notifyRandomCrashCausedByThreadSleep() {
        ...
        Log.w("UiTest", "@warning tests sometimes crash at initial launch with `No such thread for suspend` error due to `Thread.sleep()` usage. Unfortunately, there is no exact alternative.");
    }
    ...
    public void assertNotificationHeadsup(String title, String content, long notifTimeout) {
        ...
        // Check notification heads up is appearing.
        notificationPOM.awaitNotification(title, content, notifTimeout);
        notificationPOM.assertNotificationHeadsup(title, content);
    }


    public void assertNotification(String title, String content, long notifTimeout) {
        // Set up notification page object model.
        // 1. Get app name.
        Context context = InstrumentationRegistry.getInstrumentation().getTargetContext();
        String appName = context.getString(R.string.app_name);
    ...
    public void openAppInForeground() {
        // Set up Flux page object model.
        FluxPOM fluxPOM = new FluxPOM(device, timeout);

        // Open the app or put it back in foreground if already opened.
        fluxPOM.launchApp();
    }
    ...
    public void clearAllNotifications() {
        ...
        notificationPOM.openNotificationPanel();
        notificationPOM.clearAllNotifications();
