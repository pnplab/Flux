 ...
 */
public class ClientInitProvider extends ContentProvider {

    // WritableStore the android context statically so we can share it to other
    // android's component.
    static private Context _context = null;
    private final AbstractLogger _log = new DefaultLogger();// AbstractInitProvider.getLogger();
    ...
    static public Context getApplicationContext() {
        ...
        if (_context == null) {
            throw new IllegalStateException("ClientInitProvider's context is " +
                    "null. ClientInitProvider#getApplicationContext should be " +
