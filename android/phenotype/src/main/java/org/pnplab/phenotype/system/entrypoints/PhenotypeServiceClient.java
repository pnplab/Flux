 ...
 * any context.
 */
final public class PhenotypeServiceClient {

    private final Context _context;
    private final AbstractLogger _log;

    public PhenotypeServiceClient(Context context, AbstractLogger logger) {
        _context = context;
        _log = logger;
    }
    ...
    private boolean isServiceRunning() {
        ...
        //
        // Only relevant approach left is to ping the service using a broadcast
        // receiver. Thus having two states: Running, Pending. We're left to
        // - add a timeout manually to consider it is not running
        // - bind it (thus start it, but temporarily), inside the service
        //   flag whether onStart/onStartCommand has been triggered or not, and
        //   add a method to the service interface to check this.
    ...
    synchronized public void bindService(final BiConsumer<AbstractPhenotypeService.ClientAPI, Runnable> onSuccess, final Consumer<RuntimeException> onError) {
        ...
        if (_bindingState == BindingState.unbound) {
            ...
            Class<? extends AbstractPhenotypeService> phenotypeServiceClass = BuildConfig.PHENOTYPE_SERVICE_CLASS;
    ...
    synchronized private void _unbindAllAndTriggerError(RuntimeException error) {
        ...
        if (isBindingRefCount < 1) {
            throw new IllegalStateException("PhenotypeServiceClient#unbind should only be called if there is at least one binding ongoing");
        }
