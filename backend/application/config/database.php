...
// Dashboard database
// @note _ENV not enabled in php.ini, failing back to _SERVER instead.
// @note clear_env = no` had to be set in `www.conf` file.
$db['aware_dashboard']['hostname'] = $_SERVER['MYSQL_HOST'];
$db['aware_dashboard']['port'] = intval($_SERVER['MYSQL_PORT']);
$db['aware_dashboard']['username'] = $_SERVER['MYSQL_USER'];
$db['aware_dashboard']['password'] = $_SERVER['MYSQL_PASSWORD'];
...
$db['aware_dashboard']['db_debug'] = filter_var($_SERVER['MYSQL_DB_DEBUG'], FILTER_VALIDATE_BOOLEAN);
