...
class Aware_model extends CI_Model {
	...
	*/
	function insert( $database, $table ) {		
		log_message('debug', "#insert @ ${table}");

		if(strlen($this->input->post('device_id')) == 0 ) {
			log_message('error', '#insert No device Id specified');
			echo 'No device ID specified…';
			return;
		}

		if(strlen($this->input->post('data')) == null ) {
			log_message('debug', "#insert data empty @ ${table}: ");
		}
		...
		if ( $this->input->post('data') != null ) {
			...
			try {
				...
				log_message('debug', "#insert insert_batch error @ ${table}: " . $database->_error_message() );
			} catch( Exception $e ) {
				foreach( $array_data as $data ) {
					$database->insert( $table, $data );	
					log_message('debug', "#insert insert error @ ${table}: " . $database->_error_message() );
				}
			}
		}
	}
	...
	*/
	function create_table( $database, $table ) {
		log_message("debug", "#create_table ${table}");

		if( strlen($this->input->post("device_id")) == 0 ) {
			log_message("debug", "#create_table ${table} No device Id specified...");
			echo "No device ID specified";
			return;
		}

		if( $this->input->post('fields') == null ) {
			log_message("debug", "#create_table ${table} fields param empty.");
		}
		
		log_message("debug", "#create_table ${table} table_exists()");
		$table_exists = $this->tableExists($database, $table);
		log_message("debug", "#create_table ${table} table_exists/list_tables error: " . $database->_error_message() );

		if( $table_exists ) {

			log_message("debug", "#create_table ${table} table_exists()==true");
...
*/
			...
			log_message("debug", "#create_table ${table} table_exists()==false");
			...
			if( $this->input->post('fields') != null ) {
				...
				foreach($columns as $c) {
					...
					// Added for pnplab survey plugin
					if( strcasecmp("form_id", $first_word) == 0 ) {
						$c = str_replace("text", "varchar(150)", $c );
					}
					if( strcasecmp("question_id", $first_word) == 0 ) {
						$c = str_replace("text", "varchar(150)", $c );
					}
				...
				log_message("debug", "#create_table ${table} query ->");
				log_message("debug", $query);
				echo $query;
				$database->query($query);
				log_message("debug", "#create_table ${table} query: " . $database->_error_message() );
